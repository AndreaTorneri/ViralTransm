...
# according to an exponential, logistic or constant decrease
#
#
#
...
nCov.InfMeasure.treatment<-function(t, vload.tt){
  y<-seq(0,30,0.01)
...
nCov.InfMeasure.treatment.log<-function(t, vload.tt){
  b<-6
  a<-vload.tt/(1-exp(-b))
  c<-0.7
  vload.comp<-a*(1-exp(-b*exp(-c*t)))
  return(vload.comp)
}

nCov.InfMeasure.treatment.k<-function(t, vload.tt){
  m<--0.1785714
  vload.comp<-vload.tt*(1+m*t)
  return(vload.comp)
}
...
nCov.simulator.ExpHosp.QuarIso.aftersympt<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R.a,R.s,avg.dt,eta, rate.quar,s.undiagn, seeds){
  ...
  first.cases<-sample(1:n,seeds, replace = FALSE)
  for (j in first.cases){
    ...
    if (runif(1)<rho){
      if (runif(1)<sigma){ #check whether are severe
        ...
        quarantine.day[first]<-status.matrix[first,5]+nCov.diagnosis(avg.dt = avg.dt)
        status.matrix[first,6]<-quarantine.day[first]
        if (runif(1)<s.undiagn){
          quarantine.day[first]<-Inf
        }
        ...
        if (status.matrix[infectee,1]==0 & runif(1)<acc.rate){
          ...
          if (runif(1)<rho){
            if (runif(1)<sigma){ # whether are severe
              ...
              status.matrix[infectee,6]<-quarantine.day[infectee]
              if (quarantine.day[infectee]!=status.matrix[infectee,5]){
                if (runif(1)<s.undiagn){
...
nCov.simulator.ExpHosp.QuarIso.beforesympt.testing<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R.s,R.a,avg.dt, eta, rate.quar, tfailtest, tdelaytest, s.undiagn, seeds){
              ...
              if (rbinom(1,1,eta)==1){ 
                if (status.matrix[t,1]==1){
                  if ((current.time -status.matrix[t,2])<(tfailtest)){ #infectives that failed the test 
                    delay.testing[t]<-min((current.time+tdelaytest),status.matrix[t,6]) # are re-tested, if not showing symptoms before
                  }else{
...
nCov.simulator.ExpHosp.QuarIsoTreat.beforesympt.testing<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R.s,R.a,avg.dt, eta, rate.quar, tfailtest, tdelaytest, s.undiagn, seeds){
  status.matrix <- matrix(NA,nrow = n,ncol = 9) #matrix containing information about the state of the individuals
  col.name<-c("infected","time.of.infection","infector", "severity", "TimeSymptomOnset","TimeQuarantine", "Treatment", "TimeTreatment","Traced")
  dimnames(status.matrix)<-list(NULL,col.name)
  status.matrix[,1]<-0 #all the population is initially susceptible
      ...
      if (Q<=min(T_NextCtc,R_a)){ # at time of diagnosis isolation/quarantine and treatment are performed
        ...
        if (T_NextCtc<R_a){
          ...
          if (status.matrix[infector,7]==1){
            acc.rate<-min(nCov.InfMeasure.treatment(t=(current.time-status.matrix[infector,8]),vload.tt = nCov.InfMeasure(status.matrix[infector,8],lengthI =transmission.parameters$total_infectionPeriod[infector] ))*transmission.parameters$q[infector],nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]) 
          }else{
            acc.rate<-nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]
          }
          if (acc.rate>1){err<-err+1}
...
nCov.simulator.ExpHosp.QuarIsoTreat.beforesympt.testing.log<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R.s,R.a,avg.dt, eta, rate.quar, tfailtest, tdelaytest, s.undiagn, seeds){
  status.matrix <- matrix(NA,nrow = n,ncol = 9) #matrix containing information about the state of the individuals
  col.name<-c("infected","time.of.infection","infector", "severity", "TimeSymptomOnset","TimeQuarantine", "Treatment", "TimeTreatment","Traced")
  dimnames(status.matrix)<-list(NULL,col.name)
  status.matrix[,1]<-0 #all the population is initially susceptible
    ...
    if (t.testing<min(Q,T_NextCtc,R_a)){ #Time at re-test. Isolation/quarantine and treatment are both performed
      ...
      if (Q<=min(T_NextCtc,R_a)){ # at time of diagnosis isolation/quarantine and treatment are performed
        ...
        if (T_NextCtc<R_a){
          ...
          infectee<-sample(setdiff(1:n,infector),1)
          if (status.matrix[infector,7]==1){
            acc.rate<-min(nCov.InfMeasure.treatment.log(t=(current.time-status.matrix[infector,8]),vload.tt = nCov.InfMeasure(status.matrix[infector,8],lengthI =transmission.parameters$total_infectionPeriod[infector] ))*transmission.parameters$q[infector],nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]) 
          }else{
            acc.rate<-nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]
          }
          if (acc.rate>1){err<-err+1}
...
nCov.simulator.ExpHosp.QuarIsoTreat.beforesympt.testing.k<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R.s,R.a,avg.dt, eta, rate.quar, tfailtest, tdelaytest, s.undiagn, seeds){
  status.matrix <- matrix(NA,nrow = n,ncol = 9) #matrix containing information about the state of the individuals
  col.name<-c("infected","time.of.infection","infector", "severity", "TimeSymptomOnset","TimeQuarantine", "Treatment", "TimeTreatment","Traced")
  dimnames(status.matrix)<-list(NULL,col.name)
  status.matrix[,1]<-0 #all the population is initially susceptible
    ...
    if (t.testing<min(Q,T_NextCtc,R_a)){ #Time at re-test. Isolation/quarantine and treatment are both performed
      ...
      if (Q<=min(T_NextCtc,R_a)){ # at time of diagnosis isolation/quarantine and treatment are performed
        ...
        if (T_NextCtc<R_a){
          ...
          infectee<-sample(setdiff(1:n,infector),1)
          if (status.matrix[infector,7]==1){
            acc.rate<-min(nCov.InfMeasure.treatment.k(t=(current.time-status.matrix[infector,8]),vload.tt = nCov.InfMeasure(status.matrix[infector,8],lengthI =transmission.parameters$total_infectionPeriod[infector] ))*transmission.parameters$q[infector],nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]) 
          }else{
            acc.rate<-nCov.InfMeasure(t=(current.time-status.matrix[infector,2]), lengthI = transmission.parameters$total_infectionPeriod[infector])*transmission.parameters$q[infector]
          }
          if (acc.rate>1){err<-err+1}
