...
nCov.simulator.ExpHosp.QuarIsoAfter.Treatment.beforesympt.delaytesting<-function(n, lambda, rho, sigma,avg.inc, mu.IP,R,avg.dt, eta, rate.quar, tfailtest, tdelaytest){
  ...
  err<-0
  while(sum(infectives, na.rm = TRUE) > 0){ #while there are still infectives
    for (i in which(index.contact==1) ){ # for all the individuals that has to propose a  contact
      temp.contact.time<-rexp(1,transmission.parameters$contact_rate[i])+current.time
      index.contact[i]<-0
    ...
    ifelse(length(which(is.na(contact.time$pr.ctc)==FALSE))>0,T_NextCtc<-min(contact.time$pr.ctc, na.rm = T),T_NextCtc<-Inf) 
    ifelse(length(which(!is.infinite(quarantine.day)))>0,Q<-min(quarantine.day),Q<-Inf ) #minimum diagnosis time
    R_a<-min(recovery.vector, na.rm = T) # minimum among the recovery times
    ifelse(length(which(!is.infinite(delay.testing)))>0,t.testing<-min(delay.testing),t.testing<-Inf ) #minimum re-testing time
    
    
    if (t.testing<min(Q,T_NextCtc,R_a)){
      current.time<-t.testing
      tested.individual<-which(delay.testing==t.testing)
      for(t.temp in tested.individual){
        if (status.matrix[t.temp,7]==0){ #if not already treated
          status.matrix[t.temp,7]<-1 # treatment
          status.matrix[t.temp,8]<-current.time #time of treatment
        }
        quarantine.day[t.temp]<-status.matrix[t.temp,5] #quarantine/isolation at time of onset of symptoms
        delay.testing[t.temp]<-Inf
      }
    }else{
      if (Q<=min(T_NextCtc,R_a)){
        ...
        if (status.matrix[person.quarantined,4]==2){
          ...
          transmission.parameters$contact_rate[person.quarantined]<-rate.quar
          if (status.matrix[person.quarantined,7]==0){ #if not yet treated
            status.matrix[person.quarantined,7]<-1 #treated
            status.matrix[person.quarantined,8]<-current.time # time of treatment
          }
        }
        #Contact-tracing: with a certain probability, select a previously contacted individual and treat him/her 
        if (status.matrix[person.quarantined,9]==0){
          ...
          if (length(contacted.individuals)>0){
            for (t in unique(contacted.individuals)){
              ...
              if (rbinom(1,1,eta)==1){ #succesful tracing 
                if (status.matrix[t,1]==1){
                  if ((current.time -status.matrix[t,2])<(tfailtest)){ #if negative retested
                    delay.testing[t]<-min((current.time+tfailtest),status.matrix[t,6])
                  }else{ # if positive
                    quarantine[t]<-status.matrix[t,5] #quarantine/isolation at symptom onset
                    if (status.matrix[t,7]==0){ # Treatment if not already treated
                      status.matrix[t,7]<-1
